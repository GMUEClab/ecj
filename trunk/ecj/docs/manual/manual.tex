...
\begin{enumerate}
\item The \method{restoreFromCheckpoint(...)} method is called on the Checkpoint class.
\item This method then unserializes the EvolutionState from the checkpoint file.
\item This method then calls \method{resetFromCheckpoint(...)} on the EvolutionState.  The resetFromCheckpoint method normally does two things:
	\begin{enumerate}
	...
	\end{enumerate}
\item Evolution is resumed by calling the \method{run(...)} method on the EvolutionState.
\item This method then calls \method{startFromCheckpoint(...)} on the EvolutionState.  This is a simple hook method you can use to set up things before evolution starts again.
\end{enumerate}
...
When ECJ is in control, usually the subordinate library is being used to assess the quality of an Individual, perhaps as a simulation library.  To do this, you'll probably need a place to set up your library, prepare a simulation to test a string of Individuals, reset the simulation for each Individual, and eventually destroy the library.  Note that if you use checkpointing, your library must be serializable.   Note also that if you use distributed evaluation, the library will be run on remote machines, so you'd want to do this stuff on the {\bf slave}, not on the {\bf master}.

Here are some plausible locations for these tasks:
 
\paragraph{Set up the library}
One option would be to create a custom EvolutionState, overriding the \method{startFresh()} and \method{startFromCheckpoint()}.  See Section~\ref{ec.EvolutionState}.  Let us presume you don't need to distinguish between starting fresh and restarting from a checkpoint.  Then perhaps you could do in \method{setup()} like this:
...
I'd also use the \method{setup(...)} method to create an array of simulators to keep track of them.  Let's say your simulation is of class \class{Simulation}.  In your \class{MyEvolutionState} class you might add the following instance variable to hold one simulation per eval thread:

\begin{verbatim}
Simulation[] simulations = new Simulation[state.evalthreads];
\end{verbatim}

\paragraph{Prepare the simulation for evaluation}

Next you'd probably want to set up a simulation object in your thread.  The obvious time to do this is in the \method{prepareToEvaluate(...)} method (see Section~\ref{ec.Problem}):
...
Here you could create a simulation for your thread and store it.  Here'd be a good place to create and/or reset your simulation:
...
If you run ECJ multithreaded, ECJ may make several Problems and run them simultaneously in multiple threads, so be sure that your simulation does not share any data in common (notably non-final static variables).  

If you're running with distributed evaluation, this method will only get called on the slave (which is where you want it).

Notice that to construct the simulation, we're passing in the random number generator for the thread, which would be the best practice.  We're also passing in the thread number, which is a unique integer from 0 ... \variable{state.evalthreads}), which might be useful for the simulation to distinguish itself from others (for example, so it can write out some kind of statistics file as "foo.threadnumber.out" or whatever so none of the simulations overwrite each other).
...
\begin{itemize}
\item Build a new simulator each time an individual is evaluated.  If this is the case, you could say:

\begin{verbatim}
        simulations[thread] = new Simulation(state.random[thread], thread, ...);
\end{verbatim}

In this case, don't bother creating the Simulation in \method{prepareToEvaluate(...)} like we did above.

\item ``Reset'' your already-build simulator.  This is what I suggest you do, it's probably much cheaper than constructing one from scratch.  So you'd say something like

\begin{verbatim}
        simulations[thread].reset(...);
\end{verbatim}
...
After a thread has been used to evaluate a chunk of individuals (commonly once per generation), ECJ calls a special method you might use to delete and clean up all of your simulators in preparation for the next generation.  This is an unusual need: I wouldn't do this: instead, I'd just let them reset as discussed earlier and reuse them.  But if you must, here's where it'd go:
...
SimpleBreeder has options for reducing the size of each subpopulation with each generation.  If you would like Subpopulation 0 (say) to reduce by 50 individuals per generation, down to no less than 100 individuals, you can say:

\params{%
breed.reduce-by.0 = 50
breed.minimum-size.0 = 100
}

Regardless of what you provide for the minimum size, the reduction facility will never reduce to less than is necessary to fill the subpopulation with elites.  However, it may reduce to exactly the elites themselves, with no room for children (which probably isn't too useful).  So you need to make sure you set the minimum size to something appropriate.
...
\item \class{ec.breed.UniquePipeline} tries hard to guarantee that all individuals it produces are different from individual in the past generation's subpopulation.  It does each generation by building a set of individuals in the previous generation's subpopulation, then checks newly generated individuals against this set.  If they match, it discards them and tries again.

UniquePipeline will try \(N\) times to fill a certain requested number of individuals.  If it fails after \(N\) attempts, it will fill the remaining number with non-unique generated individuals.  The value of \(N\) is determined with the parameter:

\params{%
{\it base}.duplicate-retries = 20
}

How may individuals must be generated for UniquePipeline to be satisfied?  If the following parameter is true, then UniquePipeline will always fill to the maximum number permitted; else it will fill to at least the minimum number, or whatever its subsidiary pipeline provides it.  The default is \parameter{false}:
