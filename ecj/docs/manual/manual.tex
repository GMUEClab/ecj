...
\section{Jobs and the Evolve Top-level}
\label{jobs}
...
What if you need more job complexity?  For example, what if you want to run ECJ with 10 different parameter settings?  This is up to you to write.  But it's not that hard: though ECJ's top-level bootstrap code is fairly complex, it needn't be.  It's only like this to provide job facilities and other gizmos.  

In fact, to run ECJ you only need to do the following.  First, try to load an \class{ec.EvolutionState} from a checkpoint file and call \method{run(...)} on it.  Failing this, load a parameter database, possibly modify the parameter database to change some parameters, initialize an \class{ec.EvolutionState} from the database, and call \method{run(...)} on it.  In either case, when you're finally done, clean up and exit.
...
\small\begin{verbatim}
     public static void main(String[] args)
             ...
             if (state!=null)  // loaded from checkpoint
                     ...
                     // here you might modify the parameter database, then...
...
To handle different parameters, you could put this code in an outer loop which modifies the parameters in the parameter database each time around.  The source code for \class{ec.Evolve} is very heavily commented with examples and ideas along these lines to help you out.  Check it out!

In truth though, in all my experiments, I have personally {\it always} handled different parameter settings on the command line using a UNIX script.  It's much simpler than mucking with Java code.  For example, to run ten runs each of five different population sizes, perhaps you could do this (in the \file{tcsh} shell language):
...
\begin{itemize}
\item {\bf Floating Point} (\variable{float, double}): allows uniform gene randomization (\parameter{reset}), Gaussian mutation (\parameter{gauss}), or Polynomial mutation(\parameter{polynomial}).  Uniform gene randomization simply sets the gene to a random value between its minimum and maximum legal values.  To use it, you say:
...
You can use any integer \(\geq 0\) for the distribution index.
%%END PAUL
...
Our GPData object needs to hold the return value from a child.  You also need to implement a single method which copies the data from your GPData object into another, then returns it:

\java{%
public abstract GPData copyTo(GPData other);
}

All this couldn't be simpler:
...
We could modify this to include a boolean data type as well, but we'll just use the \variable{val} variable to store both boolean and real-valued data.  Let's define our three new GPNodes.  First our If-statement:
...
\item The GESpecies holds a {\bf grammar}, loaded from a file, which we will interpret according to the bytes in the Individual.
...
The GEProblem doesn't actually do the translation from list to tree.  Instead, it calls on the GESpecies to do this dirty work.  First, let's set the GESpecies, GEIndividual, and KozaFitness of Species 0:
