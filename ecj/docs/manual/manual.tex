...
This all assumes that the parameter file is a free-standing file in your filesystem.  But it might not be: you might want to start up from a parameter file stored within a Jar file (for example if your ECJ library is bundled up into a Jar file like \file{ecj.jar}).  To do this you can specify the parameter file as a file resource relative to the \file{.class} file of a class (a-la Java's \method{Class.getResource(...)} method):
 ...
 \script{%
java ec.Evolve -from \textit{myParameterFile.params} -at \textit{relative.to.this.Classname}
}

You can also say:

 \script{%
java ec.Evolve -from \textit{myParameterFile.params}
}

In which case ECJ will assume that the class is \class{ec.Evolve}.  See Section~\ref{parameterdatabase} for more discussion about all this.
...
public class ec.app.myexample.MySimpleEvolutionState extends ec.simple.SimpleEvolutionState
    ...
    public void startFresh()
        ...
        //                              randomSeedOffset, job, runtimeArguments, checkpointPrefix, checkpointDirectory
...
Last but not least, certain Breeders allow you to change the subpopulation breeding order to a {\it sequential} one.  Let's say you have 3 subpopulations.  If you turn on the following parameter:
...
The first variable (\variable{trials}) is used to maintain the results of each trial performed.  It's only used during evaluation, and then set to \variable{null} again.  Most commonly each trial is stored as a \class{java.lang.Double}, with higher values being considered better.  You can change this but if you do, be sure to read Section \ref{performingdistributedevaluationwithcoevolution} first.  {\bf Even if your trials are not \class{java.lang.Double}, they must be immutable: their internals cannot be modified, so they can be pointer-copied and not just cloned.}

The second variable (\variable{context}) is used to maintain the context of the best trial discovered for the Individual.  It's typically only used by Cooperative Coevolution, where it's important to retain not only the performance of the Individual (his Fitness), but which collaborating Individuals made it possible for him to achieve that Fitness.  If  \variable{context} is used, it's assumed that the slots of \variable{context} hold each collaborator, except for the Individual himself, whose slot is set to null.  Section \ref{maintainingcontext} discusses the issue of context in more detail.
