...
\\
\\
\large\rm {\bf Manual Version 20.3}\\
\large\rm October, 2012\\
...
Once your parameter file is inside a Jar file, you can still define a parent in {\it another} Jar file.  You just need to specify another class for ECJ to start at, and a path relative to it, like this:
...
This is a crude mechanism for changing the verbosity of the log.  If you set this flag to \variable{true}, the log will not print anything at all.   ECJ used to have an elaborate verbosity mechanism inspired from lil-gp.  This has been since deprecated since in all the years that I've used the toolkit, I'd never changed the verbosity once.  Now we have a cruder procedure for those rare cases when you need to guarantee that an ECJ process shuts up completely.  (Note that this is in flux and may change in future versions of the software).
...
I instead prefer to first copy the original so as to keep a completely separate version in case there are multithreading issues.  Here we use \class{ec.util.DataPipe} to copy the ParameterDatabase, because it's presently not \class{Cloneable} (this may change later):
...
(and so on).   Now you set up the \class{Output} class.  At this point you may wish to muzzle \variable{stdout} and \variable{stderr} if you don't want ECJ polluting them:
...
public class X extends GPNode {
    ...
    public int expectedChildren() { return 0; }  // for good measure
...
public class Sin extends GPNode {
    ...
    public int expectedChildren() { return 1; }  // for good measure
    public void eval(EvolutionState state, int thread, GPData input,
              ADFStack stack, GPIndividual indivdiual, GPProblem problem) {
        MyData data = (MyData) input;
        MyProblem prob = (MyProblem) problem;
...
public class Mul extends GPNode {
    ...
    public int expectedChildren() { return 2; }  // for good measure
    public void eval(EvolutionState state, int thread, GPData input,
              ADFStack stack, GPIndividual indivdiual, GPProblem problem) {
        MyData data = (MyData) input;
        MyProblem prob = (MyProblem) problem;
...
\begin{itemize}
\item The representation is an arbitrarily long list of bytes (See Section~\ref{list}), implemented as a special subclass of ByteVectorIndividual called \class{ec.gp.ge.GEIndividual}.\footnote{A consequence of subclassing ByteVectorIndividual is that a given grammatical rule can have no more than 256 different clauses (choice points).}
...
GESpecies starts working through the byte array and the grammar, depth-first. Each time GESpecies comes across a point in the  grammar where multiple expansions are possible, it consults the next byte in the byte array.  Let's say there are four possible expansions of the head \lisp{\(<\)foo\(>\)}, and the next byte says 145.  We compute \(145 \mod 4 = 1\) and so pick expansion 1 (the second expansion --- the first one is expansion 0).
...
\mthd{public int makeTree(EvolutionState \textit{state}, GEIndividual \textit{ind}, GPTree \textit{tree}, int \textit{position}, int \textit{treeNum}, int \textit{threadnum,}}
\hsp{\fill}{\sf HashMap \textit{ERCmap}})\\
Builds a GPTree from a GEIndividual.  The tree is stored in {\it tree}.  The tree number (and thus the particular grammar to be used) is defined by {\it treeNum}.  Bytes are read from the GEIndivdidual starting at \textit{position}.  When the tree has been generated, the first unread byte position is returned.  If the tree could not be built because there were not enough bytes, then \variable{ec.gp.ge.GESpecies.BIG\_TREE\_ERROR} is returned instead.  If \textit{ERCmap} is non-null, then as the tree is being built, any ERCs appearing the tree are registered in the ERCmap as the key-value pair \(\langle \text{\it Gene Value} \longrightarrow \text{\it ERC}\rangle\) where \textit{Gene Value} is the gene value used (a byte), and \textit{ERC} is the a \class{ec.gp.ERC} object which has the same value as the actual ERC object used.  You should treat this map as read-only.  If you don't care about any of this, just pass in null.
...
Builds an entire array of GPTrees, sufficient to create an entire GPIndividual, from a GEIndividual.  The trees are stored in {\it trees}.  Bytes are read from the GEIndivdidual starting at position 0.  When the tree has been generated, the first unread byte position is returned.  If the trees could not be built because there were not enough bytes, then \variable{ec.gp.ge.GESpecies.BIG\_TREE\_ERROR} is returned instead.  If \textit{ERCmap} is non-null, then as the tree is being built, any ERCs appearing the tree are registered in the ERCmap as the key-value pair \(\langle \text{\it Gene Value} \longrightarrow \text{\it ERC}\rangle\) where \textit{Gene Value} is the gene value used (a byte), and \textit{ERC} is the a \class{ec.gp.ERC} object which has the same value as the actual ERC object used.  You should treat this map as read-only.  If you don't care about any of this, just pass in null.
...
\paragraph{Handling ERCs}  When the GESpecies needs to produce an ERC from the grammar, it consults the next byte in the array.  If the byte is 27, it looks up 27 in a special hash table stored in GESpecies.  If it finds that 27 has hashed to an existing ERC, it clones this ERC and uses the clone.  Else, it creates a new ERC, calls \method{reset()} on it, stores it in the hash table with 27, clones it, and uses the clone.  This way all ERCs whose byte is 27 are the same value.
...
\noindent{\bf Warning!\quad} If you turn on compression in your master but not your slave (or vice versa), the slave will connect but then nothing will happen.  You will not get any warning about your error.
...
Next the Slave needs to know whether it should return entire Individuals or just the Fitnesses of those Individuals.  Individuals are generally much bigger than Fitnesses, and if you only return Fitnesses you can cut your network traffic almost in half.  The problem is that in some custom experiments your fitness evaluation procedure might modify the Individual (it depends on the nature of your experiment).  You'll need to state whether to return entire Individuals or not:
...
You can give your slave a name which it and the master will use when printing out connection information.  If you don't it'll just make up a random number.  To assign a name, you say:
...
ECJ has a hack that you can use in this situation.  SimpleEvaluator has some additional parameters:
...
In this variable store each collaborator, one per Subpopulation, plus one slot set to \variable{null} to represent the Individual himself.  The Fitness object will detect if the context exists (by default it's \variable{null}) and print out the context information when \method{printFitnessForHumans(...)} is called.  Fitness will also handle cloning this array (it calls \method{newFitness.setContext(oldFitness.getContext(context))}) when the Fitness is cloned, etc.  Fitness has some methods you can use rather than playing with the context directly:

\begin{methods}{ec.Fitness}
\mthd{public void setContext({\it Individual[]} cont, {\it int} index)}
Sets the context to the Individuals stored in \variable{context}, under the assumption that the slot for the primary Individual is located at \variable{cont[index]}.  Clones the Individuals before adding to them to the context.\mthd{public void setContext({\it Individual[]} cont}
Sets the context to the Individuals stored in \variable{context}, under the assumption that the slot for the primary Individual can be determined because it has been set to \variable{null}.  Clones the Individuals before adding to them to the context.
...
PSO determines the {\it neighbors} or {\it informants} of a particle in one of two ways.  Both require that you specify a {\it neighborhood size} (the number of informants to the individual), for example:

\params{%
breed.neighborhood-size = 11
}

Then you can either have ECJ select informants at {\it random without replacement}, or select the informants toroidally closest (on both sides) to the individual's position in the subpopulation:
...
You'll also need to specify the breeder and individual:
...
The rest of the paramters are fairly standard, though since we're using FloatVectorSpecies, we'll need to specify a pipeline and various mutation and crossover methods even though we don't use them.  We can pick dummy stuff for this.  Here's an example of a full working PSO parameter file:
...
Next you need to create the meta-level file.  Let's call this file {\bf meta.params}.   Here's one possibility.  We'll start with certain basic items (it won't run yet):
...
Let's start by defining our meta-level individual to be a DoubleVectorIndividual, of 5 numbers (for the 5 parameters above), with some default mutation and crossover information:
...
We start with the number of parameters and the genome size (which will be the same thing):
...
We start with the first parameter.  It's a double, so its type is \parameter{float} (for ``floating-point type'').  We'll use the default mutation settings for this gene, so we don't specify anything special.
...
The standard deviation parameter is also a floating-point type like the first parameter was.
...
The mutation distribution index parameter is an integer which can range from 0 to 10, hence our max gene value below (remember that the min gene value was declared in the defaults already to be 0).  Unlike the mutation-type parameter, which has three unrelated strings, here the integers have an explicit ordering: 5 is closer to 6 than it is to 9.  For integer values of this kind it's more appropriate to use something like integer-random-walk mutation than integer-reset mutation.
...
In every case, note that we define both parameters (\parameter{eval.problem.\textit{n}....}) and corresponding gene information (\parameter{pop.subpop.0.species...\textit{n}}).  You need to make sure that the right kind of parameter types match up with the right kinds of gene types.
...
Next, your base-level ECJ process will write all sorts of things to the screen.  The aforementioned \parameter{stat.muzzle} parameter will quiet some of them but not all of them.  To completely shut the base-level ECJ process up, you can set up things so that the stdout and stderr logs of the base-level ECJ process are completely muzzled.  This is defined at the meta-level:
...
Not complicated!  However, in addition to various muzzling parameters (\parameter{stat.muzzle}, \parameter{eval.problem.muzzle}, etc) you probably had added to your meta parameter file, you might also want to add the following to your slave parameter file:
...
The problem is that distributing the tests happens {\it after} the distribution out to the slaves.  We need a way to break up the tests before slave distribution.  The simplest way to do this is to use a different way of doing tests: SimpleEvaluator's \parameter{num-tests} mechanism.  We turn off tests in the MetaProblem (set it to 1) and instead turn on tests in the SimpleEvaluator.  SimpleEvaluator also gives us options about how to merge those tests: we'll stick with averaging like before:
