...
If you are using more than one subpopulation, you might be interested in knowing the fitness values on a per-subpopulation basis.  This can be done by adding the following parameter:

\params{%
stat.child.0.gather-subpops = true
}

With this parameter, we get:
...
A note about subpopulations.  Sometimes after a run certain Individuals will not have been evaluated.  This most often happens in certain coevolution situations, such as coevolution with sequential breeding, where whole subpopulations may not have been evaluated on the first generation.  In this situation, the ``mean fitness of the entire population for this generation'' (for example) will only include those individuals which were actually evaluated.

SimpleShortStatistics has an additional option for including more statistics.  If we turned on the following parameter:
...
\item (If \parameter{gather-subpops} has been turned on) {\it Once for each subpopulation...}
\begin{enumerate}
