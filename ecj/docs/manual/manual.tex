...
Evaluates the {\it individuals} in a single trial, setting their performance scores for that trial.  Each individual will be from a certain subpopulation, specified in {\it subpops}. In some versions of coevolution, only certain individuals are supposed to have their performance scores updated (the others are acting as foils).  In this case, the relevant individuals will be indicated in the {\it updateFitness} array.  Typically you'll update fitness by adding trial results to the trials ArrayList.
    ...
    public void postprocessPopulation(EvolutionState state, Population pop
        ...
        for(int i = 0; i < pop.subpops.length; i++)
            for(int j = 0; j < pop.subpops[i].individuals.length; j++) {
                ...
                fit.trials = null;      // let it garbage collect
                }
        }
    }
\end{verbatim}
