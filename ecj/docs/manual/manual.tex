...
\\
\\
\large\rm {\bf Manual Version 23}\\
\large\rm June 15, 2015\\
...
ECJ's estimation of distribution package is \package{ec.eda}.  In it there is presently one estimation of distribution algorithm: the \((\mu / \mu_W, \lambda)\) Covariance Adaptation Evolution Strategy, or CMA-ES for short.

CMA-ES maintains a distribution in the form of a multivariate Gaussian distribution.  There is significant standardized source code available for CMA-ES (see https:/\!/www.lri.fr/\(\sim\)hansen/cmaes\_inmatlab.html), and although it is a clean rewrite, ECJ's implementation tries to be fairly faithful to that code.

\subsection{CMA-ES}

The CMA-ES facility in ECJ consists of three classes: \class{ec.eda.CMAESSpecies}, \class{ec.eda.CMAESBreeder}, and \class{ec.eda.CMAESInitializer}.  The critical class is \class{ec.eda.CMAESSpecies}, which does all the hard work.  The other two largely call it to initialize the initial population (and set up the CMA-ES distribution), and to resample a subpopulation for the next generation.

As such, \class{ec.eda.CMAESSpecies} has two methods of note: \method{newIndividual(...)} and \method{updateDistribution(...)}.  The first samples a random individual from the distribution.  The second updates the distribution to reflect the fitness values among individuals in a given subpopulation.  The other big method is \method{setup(...)}, which does a lot of work because CMA-ES, unusually for a metaheuristic algorithm, has default values for many of its parameters determined not by constant numbers but by equations.
...
init =                                         ec.eda.CMAESInitializer
breed =                                        ec.eda.CMAESBreeder
pop.subpop.0.species =                         ec.eda.CMAESSpecies
pop.subpop.0.species.ind =                     ec.vector.DoubleVectorIndividual
...
In order to get CMA-ES running, we also will need to provide at least an initial mean for our distribution.  ECJ provides a few options for this.  First, you can set the mean to the {\it center} of the space (defined by the min and max gene values of your genome).  Second, you could set the mean to the origin (\(\vec 0\)) of the space.  Third, you could set the mean to a random position somewhere within the space:
...
CMA-ES has a large number of parameters: but nearly all of them are set to default values.  More importantly, and unusually, nearly all of them are set to values which are {\it computed at runtime} based on your genome size and other choices.  This means that you may not know what values are being used internally when running CMA-ES: so it will print out these parameters when it runs so you know what you're getting into.

Let's start with parameters you may wish to play with.  First, there's the \parameter{sigma}, which defines the value of \(\sigma\), a standard CMA-ES parameter which specifies the scaling of the covariance matrix.  By default this is set to 1.0, but you can change it as shown below.  Second, there's the question of how CMA-ES should terminate.  Normally ECJ terminates when it has run out of generations or evaluations, or when it has found the ideal individual.  But CMA-ES has a new wrinkle: when the eigenvalues of its covariance matrix get too small, floating-point error can cause them to go negative, at which point the whole thing goes up in smoke.  ECJ will terminate when that happens and let you know.  You can alternatively terminate early when the eigenvalues get very small, by setting \parameter{alternative-termination} to true, for example: 
...
We also have various {\bf weights} \(w_0 ... w_{\mu-1}\).  These are values used instead of the fitness values for the (sorted) best \(\mu\) individuals in the subpopulation, and affect the resulting distribution.  The default settings for each weight are \(\forall i: w_i \leftarrow \frac{\ln\left(\!\frac{\lambda + 1}{2 (i+1)}\!\right)}{\sum_{j=0}^{\mu-1} \ln\left(\!\frac{\lambda + 1}{2 (j+1)}\!\right)}\).

If you don't specify weights, ECJ will use the equation above.  Otherwise you have to specify {\it every single weight}.  Furthermore, ECJ will then renormalize your weights to guarantee that they sum to 1.

\newcommand\uf{\mu_{\text{\it eff\,}}}

Using the weights, ECJ will compute a value called \(\uf = \frac{1}{\sum_{i=0}^{\mu-1} w_i^2}\), which is used in internal computation and also in the default values of additional parameters.  Those include:
...
\item \parameter{cs}: \ \ \ \ or \(c_\sigma \leq 1 \).  This is the so-called step-size control cumulation, and is by default set to  \(\frac{\uf + 2}{n + \uf + 5}\).
\item \parameter{damps}: \ \ \ \ or \(d_\sigma \leq 1 \).  This is the so-called update dampening of the step-size control, and is by default set to  \(c_\sigma + 2 \max\left(1,\  \sqrt{\frac{\uf - 1}{n + 1}}\right) - 1\).
\end{itemize}
...
There is an example parameter file, \file{ec/app/ecsuite/cmaes.params}, located in the \file{ecsuite} application, which you can try.
