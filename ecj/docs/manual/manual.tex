...
\\
\\
\large\rm {\bf Manual Version 20.2}\\
\large\rm July, 2012\\
...
Last but hardly least, ECJ can also restart from a checkpoint file it created in a previous run:
...
\item If the Population has not read from a file, it will call \method{populate(...)} on each of its Subpopulations.  A Subpopulation's \method{populate(...)} method usually works like this.  First, it determines if it should create new individuals from scratch or if it should fill its array by reading Individuals from a file.  This is determined by the same basic procedure!  If Subpopulation 0 should be read in from the file \file{/tmp/subpopulation.in}, the parameter setting would be:

\params{%
pop.subpop.0.file = /tmp/subpopulation.in
}

Subpopulations will try to read individuals from files using \method{readSubpopulation(..., LineNumberReader)}.  If the number of individuals in the file doesn't match the size of the Subpopulation, the Subpopulation will be resized to match the file.
...
In contrast, a subpopulation file usually just has the the number of individuals for the subpopulation, followed by the individuals:
...
You can typically edit a subpopulation file out of a population file, or build a 1-subpop population file out of a subpopulation file, with some judicious typing.  Note that integers start with an ``i'' and end with a bar, as in:

\begin{verbatim}
i40|
\end{verbatim}

This oddity is due to use of ECJ's Code package (Section~\ref{ec.util.Code}). 
...
\begin{itemize}
\item \class{ec.vector.VectorGene},\footnote{The name \class{ec.vector.VectorGene} is a historical error.  It really ought to have been called \textsf{ec.vector.Gene}.} the abstract superclass of objects which can fill gene positions.
...
MasterProblem also implements all the methods defined in SimpleProblemForm and GroupedProblemForm (Section~\ref{ec.coevolve.GroupedProblemForm}), in addition to the following two methods normally implemented by Evaluator (and which are called on the MasterProblem from the Evaluator's methods of the same names):
...
Returns true if the my context is better than the other Fitness's context.  This is computed by comparing our trials: if I have trials and the other does not, or if one of my trials is superior to all of the other's trials, then returns true.  This method assumes that trials are \variable{java.lang.Double}, and that higher values are better.  If this is not the case, you'll need to override this method to customize it.
\end{methods}
