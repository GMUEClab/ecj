...
This variable is used by the coevolutionary processes (see Section \ref{ec.coevolve}) to keep track of the number of trials (in the form of \class{java.lang.Double} used to compute the Fitness value.  Outside of coevolution it's presently unused.  Ultimately the variable doesn't matter once the Fitness has been computed.
...
\item Individuals are tested against each other in various matches (or with one another in various collaborative problems).  These trials cause trial performance scores of the Individuals to accumulate.
\item The final Fitnesses of the Individuals are set based on the performance scores over all the trials.
\end{enumerate}

It's up to you to store the trial results and eventually form them into final Fitness values, as discussed later.  GroupedProblemForm will help you by defining three methods to do these various portions of the evaluation process.  You'll need to implement all three methods:
...
\label{printingjointcollaborations}
...
During the \method{evaluate(...)} method, each Individual uses this array to keep track of the collaborators which enabled the Individual to perform best.  For example, you might check to see if the most recent trials is the best one so far, and update the context if so:

{
\small\begin{verbatim}
// for purposes here let's assume we just added the 
// recent trial to the end of the 'trials' variable

double trial = ((Double)(trials.get(trials.length - 1))).doubleValue;
boolean best = true;

for(int i = 0; i < trials.length - 1; i++)
    {
    double other =  ((Double)(trials.get(i))).doubleValue;
    if (other > trial)    // we presume less than is worse than
       {  best = false;  break; }
    }
if (best)  // I'm best, store my context in the Individual
    {
    // store context individuals here
    }
\end{verbatim}
}


Then you could modify \method{printIndividualForHumans} to print out that best context, with the following hack:
{
\small\begin{verbatim}
public void printIndividualForHumans(EvolutionState state, int log)
    {
