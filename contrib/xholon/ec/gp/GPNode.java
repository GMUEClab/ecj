 ...
 * May 29, 2006
 * - GPNode extends Activity, implements IXholon
 * - in setup(), added call to setXhc()
 * - added new methods that are part of the Xholon's IXholon interface:
 *   - getFirstChild()
 *   - getNextSibling()
 *   - getName()
 *   - writeXml()
...
public abstract class GPNode extends Activity implements GPNodeParent, IXholon // ECJ 15
    ...
    */
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        setXhc(XholonClass.getClassNode(getName())); // set the Xh class of this Xholon
        }
    ...
    */
    ...
    {
    	//System.out.println(parent.getClass() + " " + parent.getClass().getSuperclass());
    	if (parent.getClass().getSuperclass() == GPNode.class) {
    		if (((GPNode)parent).children.length > argposition + 1) {
    			//System.out.println(this + " 's next sibling is " + ((GPNode)parent).children[argposition+1]);
    			return ((GPNode)parent).children[argposition+1];
    		}
    	}
    	return null;
    }
    ...
    {
    	String elementName = getClass().getName();
		elementName = elementName.substring(elementName.lastIndexOf('.') + 1);
		return "Pf" + elementName;
    	//return "GPNode_" + toString();
    }
